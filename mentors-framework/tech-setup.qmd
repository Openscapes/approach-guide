...
We use **Google Groups when possible** to easily add mentors and other cohorts of participants (e.g. NMFS Openscapes Data Academy) to Google Folders and Calendars. Google Groups work well for people and organizations that use Google products, and not so well for those who use Microsoft products. Not everyone can use their work email or computer to access Google products. We collect a second email address in ParticipantsList spreadsheets, but if we use Google Groups we need to make sure that everyone's Google-access emails are included in the Google Group.

**For NASA Mentors**, we use a Google Group for emails, but for calendar invites, we must add individuals' emails, otherwise they do not see updates in their own calendars.

**For NOAA Fisheries**, we use Google Groups successfully for emails and calendar invites. We have tested adding and deleting an individual from a Group and confirmed they see the relevant calendar invite appear and disappear as expected.
...
**Step 3:** In the Google Groups (no admin access needed; see screenshot):

-   change Group Settings, Who can see group - Anyone on the web
    -   Organization members (aka Openscapes team with Google Workspace access) or Administrator should set viewing privacy (screenshot) so conversations are not visible to the open internet.
-   Who can see conversations - the Google Group members.

![](images/google-group-setting.png){width="60%"}

The final settings (screenshot):

![Google Group access (in About)](images/google-group-access.png){fig-align="left" width="60%"}
