...
-   Invite everyone to Slack. 
  -   Use example email below; follow up immediately from inside the new private channel by adding list of emails to add people

::: {.callout-tip collapse="true"} 
## Example email Slack invitation

*It seems the only way to add new people to our Slack AND have them join a private channel is to send the invite from inside that channel. This email is key for giving background though.*
...
Slack can be a daunting new tool at a time when we're introducing lots of tools. That's why we send that welcoming invitation email providing some background and links to guidance, rather than simply adding people's emails from inside Slack.

-   Write a welcome message in the general channel

::: {.callout-tip collapse="true"}
## Example Slack #general post
